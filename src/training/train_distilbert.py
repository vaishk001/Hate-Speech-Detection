...
OUT_PATH = Path("models/baseline/logreg_distilbert.joblib")
CLEAN_CSV = Path("data/clean_data.csv")
REPORT_JSON = Path("reports/classification_report_distilbert.json")
CONFUSION_PNG = Path("reports/confusion_matrix_distilbert.png")
...
def main(args):
    if not MODEL_DIR.exists():
        raise FileNotFoundError(f"DistilBERT dir not found: {MODEL_DIR}. Run scripts/download_transformer.py while online.")
    ...
    print(f"Saved DistilBERT-logreg to: {OUT_PATH}")
